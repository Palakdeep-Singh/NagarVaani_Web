...
export const fastLoginService = async (role) => {
  if (role === 'citizen') {
    ...
    if (error || !admin || admin.length === 0) {
      // Fallback mock users for hackathon demo — works for all roles even with empty admins table
      const MOCK_ADMINS = {
        'ECI': { id: 'MOCK-ECI', email: 'eci@eci.gov.in', name: 'ECI Admin', role: 'ECI', state: null, district: null, designation: 'Chief Election Commissioner' },
        'CEO': { id: 'MOCK-CEO', email: 'ceo.up@eci.gov.in', name: 'CEO Uttar Pradesh', role: 'CEO', state: 'Uttar Pradesh', district: null, designation: 'Chief Electoral Officer' },
        'DEO': { id: 'MOCK-DEO', email: 'deo.lucknow@eci.gov.in', name: 'DEO Lucknow', role: 'DEO', state: 'Uttar Pradesh', district: 'LUCKNOW', designation: 'District Election Officer' },
        'Returning Officer': { id: 'MOCK-RO', email: 'ro.lko1@eci.gov.in', name: 'RO AC-01 Lucknow', role: 'Returning Officer', state: 'Uttar Pradesh', district: 'LUCKNOW', designation: 'Returning Officer' },
        'Sector Officer': { id: 'MOCK-SO', email: 'so.sector12@eci.gov.in', name: 'SO Sector-12', role: 'Sector Officer', state: 'Uttar Pradesh', district: 'LUCKNOW', designation: 'Sector Officer' },
        'Presiding Officer': { id: 'MOCK-PRO', email: 'pro.booth145@eci.gov.in', name: 'PrO Booth-145', role: 'Presiding Officer', state: 'Uttar Pradesh', district: 'LUCKNOW', designation: 'Presiding Officer' },
        'Polling Officer': { id: 'MOCK-PO', email: 'po.booth145@eci.gov.in', name: 'PO-1 Booth-145', role: 'Polling Officer', state: 'Uttar Pradesh', district: 'LUCKNOW', designation: 'Polling Officer' },
      };
      const mock = MOCK_ADMINS[role];
      if (!mock) throw new Error(`Unknown role "${role}"`);
      const token = signJWT({ adminId: mock.id, role: mock.role, state: mock.state, district: mock.district });
      return { success: true, token, admin: { ...mock, phone: '9999999999', is_active: true }, role: mock.role };
    }
