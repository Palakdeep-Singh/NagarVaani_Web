...
export default function PollingOfficerDashboard({ user, onLogout }) {
  const userName = user?.name || 'Polling Officer';
  const userRole = user?.role || 'PO';

  // Determine booth number from employeeId e.g. PO-B101-1 → 101
  const boothNum = (() => {
    const match = (user?.employeeId || user?.assignedBooth || '').match(/B(\d+)/i);
    return match ? parseInt(match[1], 10) : 101;
  })();
  ...
  // Local voter roll (static demo - real EVM integration would need ECIL APIs)
  const [voterRoll] = useState([
  ...
  // Accessibility requests (logged locally for now)
  ...
  const [poMessage, setPoMessage] = useState({
    ...
    time: "–",
    status: "New",
    text: "Loading directives from Presiding Officer..."
  });
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get(`${API_BASE}/booth/status?booth_id=${boothNum}`, { headers });
      const data = res.data;
      setBoothData(data);

      // Sync checklist from backend
      // (returned as data.checklists)

      // Sync directives as PO messages
      if (data.returningOfficerRequests?.length > 0) {
        const latest = data.returningOfficerRequests[0];
        setPoMessage({
          sender: 'Presiding Officer / RO',
          time: latest.time,
          status: latest.status === 'Pending' ? 'New' : latest.status,
          text: latest.text,
          id: latest.id,
        });
        setPoMessagesList(data.returningOfficerRequests.map(d => ({
          sender: 'Presiding Officer',
          text: d.text,
          time: d.time,
          status: d.status,
      ...
      console.error('[PO Dashboard] fetchData error:', err);
    } finally {
  ...
  // ── Derived live values ───────────────────────────────────────────────
  const votersProcessed = boothData?.turnout?.voted ?? 0;
  const totalVoters = boothData?.turnout?.total ?? 1287;
  const turnoutPct = boothData?.turnout?.percentage ?? 0;
  const queueCount = boothData?.queueCount ?? 0;
  const evmStatus = boothData?.evmStatus ?? 'Operational';
  const evmBattery = boothData?.evmBattery ?? 100;
  const boothName = boothData?.name ?? `Booth ${boothNum}`;
  const boothWard = boothData?.ward ?? '';

  // Live complaints from incidents
  const liveComplaints = (boothData?.incidents || []).map(inc => ({
    id: inc.id,
    type: inc.type,
    citizen: `Incident: ${inc.id}`,
    time: inc.time,
    status: inc.status === 'Open' ? 'In Progress' : inc.status,
  }));

  // Live checklist from backend
  const liveChecklist = (boothData?.checklists || []).length > 0
    ? boothData.checklists
    : [
      { id: 'mock-poll', label: 'Mock Poll Completed', time: '07:15 AM', checked: false },
      { id: 'evm-sealed', label: 'EVM Sealed', time: '07:25 AM', checked: false },
      { id: 'voter-verification', label: 'Voter Verification Started', time: '07:30 AM', checked: false },
      { id: 'ink-entry', label: 'Ink & Entry Control', time: '07:35 AM', checked: false },
    ];

  // hourly turnout from backend
  const hourlyData = boothData?.hourlyTurnout || [];

  const handleToggleChecklist = async (id) => {
    try {
      const token = localStorage.getItem('nc_token');
      const headers = token ? { Authorization: `Bearer ${token}` } : {};
      await axios.post(`${API_BASE}/checklist/${id}?booth_id=${boothNum}`, {}, { headers });
      fetchData(); // refresh
    } catch (err) {
  ...
  const handleAcknowledgePoMessage = async () => {
    setPoMessage(prev => ({ ...prev, status: 'Acknowledged' }));
    try {
      const token = localStorage.getItem('nc_token');
      const headers = token ? { Authorization: `Bearer ${token}` } : {};
      await axios.post(`${API_BASE}/actions`, { action: 'acknowledge_ro_request', payload: { id: poMessage.id } }, { headers });
    } catch (err) { /* silent */ }
  };

  const handleCompletePoMessage = () => {
    setPoMessage(prev => ({ ...prev, status: 'Completed', text: 'No pending tasks.' }));
  };
  ...
  const handleReportIssue = async (type) => {
    try {
      const token = localStorage.getItem('nc_token');
      const headers = token ? { Authorization: `Bearer ${token}` } : {};
      await axios.post(`${API_BASE}/actions?booth_id=${boothNum}`, { action: 'report_incident', payload: { type } }, { headers });
      alert(`Issue "${type}" reported to Presiding Officer.`);
      fetchData();
    } catch (err) {
        ...
        <header className="main-header" style={{
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <h1 style={{ fontSize: '18px', fontWeight: '800', margin: 0 }}>Welcome, {userName}</h1>
              <span style={{
                fontSize: '10px',
                fontWeight: '800',
                padding: '3px 8px',
                borderRadius: '12px',
                backgroundColor: evmStatus === 'EVM Fault' ? '#dc2626' : '#22c55e',
                color: '#fff'
              }}>{evmStatus === 'EVM Fault' ? 'EVM Fault' : 'Operational'}</span>
            </div>
            <p style={{ fontSize: '12px', color: '#94a3b8', margin: '4px 0 0', fontWeight: '500' }}>
              {boothName} {boothWard ? `• ${boothWard}` : ''} • Constituency: AC-40 New Delhi
            </p>
          </div>
            ...
            <div style={{ display: 'flex', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 12px', backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: '10px' }}>
                ...
                <div style={{ fontSize: '11px', display: 'flex', flexDirection: 'column' }}>
                  ...
                  <strong style={{ color: '#fff' }}>{votersProcessed} <span style={{ color: '#94a3b8', fontWeight: 'normal' }}>/ {totalVoters}</span></strong>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 12px', backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: '10px' }}>
                ...
                <div style={{ fontSize: '11px', display: 'flex', flexDirection: 'column' }}>
                  <span style={{ color: '#94a3b8', fontSize: '9px', textTransform: 'uppercase' }}>EVM Status</span>
                  <strong style={{ color: evmStatus === 'Operational' ? '#22c55e' : '#f87171' }}>{evmStatus} ({evmBattery}%)</strong>
                </div>
              </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '20px', margin: '16px 0' }}>
                    <div style={{ position: 'relative', width: '85px', height: '85px', flexShrink: 0 }}>
                      <svg width="85" height="85" viewBox="0 0 36 36" style={{ transform: 'rotate(-90deg)' }}>
                        ...
                        <circle cx="18" cy="18" r="15.915" fill="transparent" stroke="#22c55e" strokeWidth="3.5" 
                                strokeDasharray={`${Math.min(turnoutPct, 100)} 100`} strokeDashoffset="0" />
                      </svg>
                      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                        <strong style={{ fontSize: '16px', color: '#0f172a', lineHeight: 1 }}>{turnoutPct}%</strong>
                    ...
                    <div style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '11px' }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <strong style={{ color: '#0f172a' }}>{Math.max(0, totalVoters - votersProcessed)}</strong>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #f1f5f9', paddingTop: '4px' }}>
                        ...
                        <strong style={{ color: '#0f172a' }}>{totalVoters.toLocaleString()}</strong>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                      {(liveComplaints.length > 0 ? liveComplaints : [
                        { id: 'no-data', type: 'No active incidents at this booth', citizen: '—', time: '—', status: 'Resolved' }
                      ])
