...
import VoterUploadModal from '../components/VoterUploadModal.jsx';
...
export default function PresidingOfficerDashboard({ user, onLogout, boothIdOverride, onBackToCM }) {
  ...
  // Booth core data
  const [boothId, setBoothId] = useState(boothIdOverride || 101);
  ...
  const fetchData = async () => {
    try {
      setSyncLoading(true);
      const res = await api.get(`/booth/status?booth_id=${boothId}`);
      if (res.status === 200) {
        const data = res.data;
        setIsOnline(true);
        // Map new MongoDB payload → local state
        setBoothData({
          id: data.id,
          name: data.name || `Booth ${boothId}`,
          constituency: data.constituency || 'AC-40 New Delhi',
          status: data.status === 'Healthy' ? 'Operational' : data.status,
          sectorDirective: data.sectorDirective || null
        });
        setTurnout(data.turnout || { percentage: 0, voted: 0, total: 0 });
        setQueue(data.queue || { length: 'Low', count: 0, waitTime: 0 });
        setHealthScore(data.health?.score || 80);
        setHealthChecks(data.health?.checks || { evm: true, power: true, queue: 'ok', internet: true, staff: true });
        setEvmStatus({
          id: data.evm?.id || `EVM-${boothId}`,
          status: data.evm?.status || 'Operational',
          battery: data.evm?.battery || 100,
          lastChecked: data.evm?.lastChecked ? new Date(data.evm.lastChecked).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'N/A'
        });
        // Map MongoDB incidents to complaint-style objects for the dashboard
        const dbIncidents = data.incidents || [];
        setIncidents(dbIncidents.map(i => ({ id: i.id, type: i.type, time: i.time, status: i.status })));
        setComplaints(dbIncidents.filter(i => i.status === 'Open' || i.status === 'Acknowledged').map(i => ({
          id: i.id, type: i.type, citizen: 'Reported', time: i.time, status: i.status === 'Open' ? 'pending' : 'verified', logs: [], category: i.type
        })));
        if (data.checklists) {
      ...
      console.warn('Backend server offline, running on client mock state', err.message);
      setIsOnline(false);
    } finally {
  ...
  const handleUploadVoterList = async (e) => {
    ...
    try {
      ...
      formData.append('voterList', uploadFile);
      formData.append('boothId', user?.assignedBooth || '6675306bfb8e053f3e8f6f5d'); 
      formData.append('acId', user?.assignedAC || '6675306bfb8e053f3e8f6f59');
      ...
      console.error(err);
      alert("Failed to upload voter list. Please try again.");
    } finally {
  ...
  const handleAction = async (actionType, payload = {}) => {
    try {
      ...
      // Backend API Call
      const res = await api.post(`/actions?booth_id=${boothId}`, { action: actionType, payload });
      if (res.status === 200) {
        fetchData();
      }
    } catch (err) {
  ...
  const toggleChecklist = async (id) => {
    ...
    if (isOnline) {
      try {
        await api.post(`/checklist/${id}?booth_id=${boothId}`, { checked: updated });
      } catch (err) {
        console.error(err);
      }
    }
      ...
      <main className="main-content">
        ...
        <header className="main-header">
          ...
          <div className="header-actions">
            <button onClick={() => setShowUploadModal(true)} style={{ marginRight: '8px', padding: '6px 12px', backgroundColor: '#2563eb', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>Upload Voter List (PDF)</button>
            ...
            {!boothIdOverride && (
              ...
              >
                <option value={101}>Booth 101 (Primary School R1)</option>
                <option value={102}>Booth 102 (Primary School R2)</option>
                <option value={103}>Booth 103 (Community Center)</option>
                <option value={104}>Booth 104 (Govt High School)</option>
                <option value={105}>Booth 105 (Panchayat Ghar R1)</option>
                <option value={106}>Booth 106 (Panchayat Ghar R2)</option>
                <option value={107}>Booth 107 (Vikas Bhawan)</option>
                <option value={108}>Booth 108 (Girls College)</option>
                <option value={109}>Booth 109 (Town Hall)</option>
                <option value={110}>Booth 110 (MCD School)</option>
                <option value={111}>Booth 111 (Civil Hospital)</option>
                <option value={112}>Booth 112 (Veterinary Clinic)</option>
              </select>
            )}
        ...
        <VoterUploadModal isOpen={showUploadModal} onClose={() => setShowUploadModal(false)} boothId={boothId} acId={user?.assignedAC} user={user} />
