...
export default function App() {
  const [token, setToken] = useState(() =>
    localStorage.getItem("em_auth_token"),
  );
  ...
  if (token && user) {
    if (user.role === "CM") {
      ...
      return (
        <DistrictElectionOfficerDashboard user={user} onLogout={handleLogout} />
      );
    } else if (user.role === "CEO") {
      return (
        <ChiefElectoralOfficerDashboard user={user} onLogout={handleLogout} />
      );
