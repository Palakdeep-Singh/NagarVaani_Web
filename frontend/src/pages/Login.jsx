...
export default function Login({ onLoginSuccess, onGoToPortal }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${apiUrl}/auth/admin`, {
        email: selectedRole + "@admin.gov.in",
        password: password,
      });

      if (res.data && res.data.token) {
        onLoginSuccess(res.data.token, res.data.user || res.data.admin);
      } else {
  ...
  const handleFastLogin = async () => {
    ...
    try {
      const apiUrl =
        import.meta.env.VITE_API_URL || "http://localhost:5000/api";
      const res = await axios.post(`${apiUrl}/auth/fast-login`, {
        role: fastAuthRole,
      });
